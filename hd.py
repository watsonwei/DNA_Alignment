...
def fast_sequence_alignment(s1, s2, id, **kwargs):
    '''
    ...
    try:
        filename = 'result/alignment/%s.txt' % id
        ...
        l = kwargs.pop('l',1)
        b = kwargs.pop('b', 0)
        s="./cda A B {ms} {q} {r} {l} {b} > "+filename
        cmd = s.format(ms=ms, q=q, r=r, l=l, b=b)
        # print cmd
        os.system(cmd)
        os.remove("A")
        os.remove("B")
...
def hamming_distance(s1, s2, id, gaps_ignore=False, **kwargs):
    '''
    ...
    '''
    try:
        s1, s2 = fast_sequence_alignment(s1.replace('_',''), s2.replace('_',''), id,**kwargs)
